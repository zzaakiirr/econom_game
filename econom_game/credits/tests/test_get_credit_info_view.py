...
class GetCreditInfoTests(TestCase):
    def test_make_take_credit_url_resolves_url_credit_view(self):
        view = resolve('/api/v1/get_credit_info/')
        self.assertEquals(view.func, get_credit_info)


class GetCreditInfoTestCase(TestCase):
    def setUp(self):
        ...
        self.url = reverse("get_credit_info")
        self.data = {
...
class NotOperatorAndNotSuperUserGetCreditInfoTests(GetCreditInfoTestCase):
    def setUp(self):
        super().setUp()
