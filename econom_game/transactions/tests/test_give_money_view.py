...
from ..views import give_money


User = get_user_model()


class GiveMoneyTests(TestCase):
    def test_give_money_url_resolves_give_money_view(self):
        view = resolve('/api/v1/give_money/')
        self.assertEquals(view.func, give_money)


class GiveMoneyTestCase(TestCase):
    def setUp(self):
...
class InvalidMoneyAmountFormatGiveMoneyTests(GiveMoneyTestCase):
    def setUp(self):
        super().setUp()
        self.data = {
            'card_type': 'card_number', 'card': '1',
    ...
    def test_money_not_transfered_to_team_card(self):
        self.assertEquals(
...
class NotFinancierAndNotSuperUserGiveMoneyTests(
        InvalidMoneyAmountFormatGiveMoneyTests):
    def setUp(self):
        super().setUp()
...
class SuccessfulGiveMoneyTests(GiveMoneyTestCase):
    def setUp(self):
        super().setUp()
        self.response = self.client.post(
            self.url, json.dumps(self.data), content_type="application/json"
    ...
    def test_money_transfered_to_team_card(self):
        self.assertEquals(
            self.changed_card.money_amount,
